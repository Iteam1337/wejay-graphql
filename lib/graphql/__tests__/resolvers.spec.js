...
describe('resolvers', () => {
  ...
  describe('Mutations', () => {
    ...
    describe('#joinRoom', () => {
      ...
      it('should publish a join message', () => {
        resolvers.Mutation.joinRoom(null, { input })

        expect(pubsub.publish).toHaveBeenCalledWith('usersUpdated', {
          roomName: 'test',
          usersUpdated: []
        })
      })
    ...
    describe('#search', () => {
      it('calls spotify api for searching tracks', () => {
        resolvers.Mutation.search(null, { query: 'test' })

        expect(spotify.searchTracks).toHaveBeenCalledWith('test')
      })

      it('returns search results', async () => {
        try {
          const result = await resolvers.Mutation.search(null, {
            query: 'test'
          })
